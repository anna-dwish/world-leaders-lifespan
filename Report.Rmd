...
generate_model = function(priors,model_name){
  ...
  if (file.exists(model_name)) {
    ...
    rjags_model <-  jags.parallel(data = data_leaders,  
                        ...
                        n.iter = 10000, #increase for better model output - 250K
                        n.chains = 3,
...
```{r}
lowvar <- as.data.frame(Bayesian_Leaders_output$BUGS$sims.matrix)
noninform <- as.data.frame(Bayesian_Leaders_6aii$BUGS$sims.matrix)
# hist(Bayesian_Leaders_6aii$BUGS$sims.matrix[,"beta_0"])
```

```{r}

hgA <- hist(Bayesian_Leaders_output$BUGS$sims.matrix[,"beta_0"], col='red', plot=FALSE) # Save first histogram data
hgB <- hist(Bayesian_Leaders_6aii$BUGS$sims.matrix[,"beta_0"], col='blue', plot = FALSE) # Save 2nd histogram data
