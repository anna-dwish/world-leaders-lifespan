...
Leaders in this dataset date from the 14th century through present day. Survival time, in years, is defined to be either the leader’s current age, if alive, or age of death, if deceased. The 10 leaders in the dataset that are currently alive are considered right censored observations - that is, we do not know their age of death, but know their current age. (change to: The 11 leaders that are currently alive or do not have a recorded death date are considered right censored observations - that is, we do not know their age of death, but know their current age. 10 leaders are currently alive, and Chinese emperor Jiawen, who was born in the 14th century, disappeared and is presumably long dead by now.)

AWEOFAHSKLDJFALS - also maybe say somthing about removing Obama and ppl? 
...
Using JAGS, we ran Model 1 with 500,000 simulations to determine the posterior density $\pi(r, \ \beta_0, \ \beta_1, \ \beta_{2j}, \ \beta_{3j}\, | \,data)$, with results summarized in Table 1. The 95% credible interval is created using the 2.5%  and 97.5% quantiles from the posterior distribution. 
...
time <- filtered_leaders$Birth.Date
...
ggplot(data = survivalPredicted %>% mutate(Censored = case_when(
  ...
  geom_jitter(alpha = 0.8, height = 0.5, width = 0.25) + 
...
WALEIUFHASLKFHAKLSDF  add other people in
...
gridExtra::grid.arrange(
  ggplot() + 
    ...
    annotate("text", x = age_sun +17, y = 0.0255, label = paste0("Age at Death: \n", round(age_sun,0))) + 
  ...
  ggplot() + 
    ...
    annotate("text", x = age_WC + 13, y = 0.020, label = paste0("Age at Death: \n", round(age_WC,0))) + 
