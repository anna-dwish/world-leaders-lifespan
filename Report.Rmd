  ...
  for (r in r_vals){
    ...
    for (b in b_vals){
      ...
      if (file.exists(file_name)) {
        ...
        load(file = file_name)
        df <- as.data.frame(rjags_model$BUGSoutput$sims.matrix)
        d <- density(df[,x]) # returns the density data
        r_label = paste0("R Prior: ",r)
        b_label = paste0("B Variance Prior: ", b)
        if (col == 1 && row == 0){
          plots <- append(plots,plot(d,main=b_label))
        }
        else if (col == 0 && row==0){
          plots <- append(plots,plot(d,main=b_label,ylab=r_label))
        }
        else if (col == 0){
          plots <- append(plots,plot(d,main="",ylab=r_label))
        }
        else {
          plots <- append(plots,plot(d,main=""))
        }
        col = col + 1
      }
      else{
        fail_load = paste0("Failed to load file: ", file_name)
        print(fail_load)
      }
    }
    row = row + 1
  }
  grid.arrange(plots,nrow=4,ncol=2)
}
...
for (r in r_vals) {
  for (b in b_vals) {
    file_name = paste0("leaders_model_r_",r,"_beta_",b,".Rdata")
    load(file_name)
    wait = rjags_model$BUGSoutput$sims.matrix %>% as.data.frame() %>% select(survival_Francis) %>% 
      mutate(r = expression(paste0("r = ", r, " beta = ", b)))
    sensitivity_df = sensitivity_df %>% bind_rows(wait)
  }
}
sensitivity_df %>% 
  ggplot() + 
  geom_density(aes(x = survival_Francis)) + 
