...
bind_rows(Percent_Difference_1800,
  ...
  kableExtra::kable(booktabs = TRUE,
                    caption = "Percent Difference in Mean Survival Time For World Leaders as Compared to Popes, Conditioned on Various Birth Years", 
                    col.names = c("Leadership Type", "Mean", "95% CI")) %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
...
The above table shows, at three different birth years, the expected percent difference in the mean survival time of world leaders as compared to the baseline level of Popes. Across the three birth years, 1800, 1850, and 1900, the expected survial times for Chinese Emperors and Dalai Lamas is significantly lower than that of Popes. For example, a Chinese Emperor born in 1900 is expected to have a survival time that is `r round(ce_percent_difference_ft[2], 2)` % less than a Pope born in the same year. Similarly, a Dalai Lama born in 1900 is expected to have survival time that is `r round(dl_percent_difference_ft[2], 2)` % less than a Pope born in the same year. 

Althought the average percent difference in expected survival time for Japanese Emperor and U.S. Presidents as compared to Popes is negative across the three birth years, the corresponding credible intervals contain zero, suggesting that these value are not significnt. Thus, the difference in survival times for these two groups, compared to Popes, when conditioned on specific birth years cannot be commented on concretely. 
