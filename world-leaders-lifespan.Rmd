...
#
censoring_limits <- ifelse(censored == 0, 110, Age.Event)
#
# Covariates (centered)
#
x_1 <- filtered_leaders$time - mean(leaders$time) # looking at "time" by the decade
...
present_length <- ifelse(censored == 0, 0, filtered_leaders$Age.Event)
upper_length <- 110
```
...
generate_model_text = function(priors=""){
  ...
  for (i in 1:n){ 
    ...
    y_pred_censored[i] ~ dweib(r, mu[i])T(present_length[i], upper_length)
  }
...
## Model 7a - r=gamma(25,0.2), Betas: N(0,1) - THIS IS THE GOOD ONE 
...
```{r}
#extracting posterior predictions for each individual
...
survivalPredicted <- survivalPredicted %>%
  ...
  mutate(Standardized_Residual = (mean - actual)/sd) %>%
  mutate(Residual = mean - actual) %>%
...
plot(y = survivalPredicted$Standardized_Residual, x = survivalPredicted$time_of_birth, col= survivalPredicted$Censored, pch = shapes, main = "Standardized Residuals versus Time", xlab = "Time", ylab = "Standardized Residuals")
abline(h =0)
...
shapes = c(16, 4) 
shapes <- shapes[as.numeric(survivalPredicted$Censored)]
plot(survivalPredicted$Standardized_Residual, col= x_2, pch = shapes,  main = "Standardized Residuals versus Index", xlab = "Index", ylab = "Standardized Residuals")
abline(h =0)
legend("topleft", legend = c("Popes", "Chinese Emperor", "Dalai Lama", "Japanese Emperor", "US President", "Censored"),
...
Bayesian_Leaders_V1a$BUGSoutput$summary[aa,]
Bayesian_Leaders_V1a$BUGSoutput$summary[c("survival_Francis",
                                          "survival_Obama",
                                          "survival_Naruhito",
                                          "survival_DL"), ] 
```
...
Bayesian_Leaders_V4b %>% typeof()


tidybayes::gather_draws(Bayesian_Leaders_V4b %>% as.mcmc(), mu[i], r) 
ggplot(aes(x = .value)) + 
  geom_histogram()


rweibull(n = 100,shape = 4, scale = 1.633428e-08) %>% hist()
hist(l$Age.Event)
```
...
Bayesian_Leaders_V5a$BUGSoutput$summary
tidybayes::gather_draws(Bayesian_Leaders_V5a %>% as.mcmc(), r, mu[i]) %>% 
  ungroup() %>% 
  group_by(.variable) %>% 
  filter(i ==3 | is.na(i)) %>% 
...
rweibull(1, scale = 2.249767e+00, shape = 6.474022e-05) 

ggplot(data = filtered_leaders, aes(x = Age.Event)) + geom_histogram()
```
...
Bayesian_Leaders_V7a$BUGSoutput$summary["y_pred_censored[102]",]
Bayesian_Leaders_V7a$BUGSoutput$summary["y_pred_not_censored[102]",]

tidybayes::gather_draws(Bayesian_Leaders_V7a %>% as.mcmc(), y_pred[i]) %>% 
  ungroup() %>%
  filter(i == 1) %>% 
  ggplot() + geom_histogram(aes(x = .value)) + 
  geom_vline(xintercept = filtered_leaders %>% filter(Name == "Benedict XVI" | Name == "Leo XIII") %>% pull(Age.Event))


bayesplot::ppc_stat(filtered_leaders %>% filter(Name == "John Paul II") %>% pull(Age.Event),
                    tidybayes::gather_draws(Bayesian_Leaders_V7a %>% as.mcmc(), y_pred[i]) %>% filter(i==2) %>% pull(.value) %>% as.matrix() )
```

```{r}
p1=tidybayes::gather_draws(Bayesian_Leaders_V7a %>% as.mcmc(), r, beta_0, beta_1) %>%  
  # tidyr::gather(key, value, "beta_0", "beta_1", "r", -i) %>% 
  ggplot() + 
  geom_line(aes(x = .iteration, y = .value, color = .variable)) + 
  facet_grid(.variable~., scales = "free_y")

p2=tidybayes::gather_draws(Bayesian_Leaders_V7a %>% as.mcmc(), beta_type[i]) %>% 
  filter(i != 1) %>% 
...
p3=tidybayes::gather_draws(Bayesian_Leaders_V7a %>% as.mcmc(), beta_int[i]) %>% 
  filter(i != 1) %>% 
  mutate(.variable = ifelse(is.na(i), .variable, paste0(.variable, "[",i, "]"))) %>% 
  #tidyr::gather(key, value, `beta_type[2]`:`beta_type[5]`, -i) %>% 
  ggplot() + 
  geom_line(aes(x = .iteration, y = .value, color = .variable)) + 
  facet_grid(.variable~., scales = "free_y") + 
  labs(title = "Traceplots for Interaction Effect",
       x = "Iteration", y = "Value", color = "Parameter")
# post_values %>% 
...
plot_acf <- function(parameter) {
  Bayesian_Leaders_V7a$BUGSoutput$sims.matrix %>% 
    as_tibble() %>% 
    pull(parameter) %>% 
    acf()
}
...
tidybayes::gather_draws(Bayesian_Leaders_V7b %>% as.mcmc(), y_pred[i]) %>% 
  ungroup() %>%
  filter(i == 1) %>% 
  ggplot() + geom_histogram(aes(x = .value)) + 
  geom_vline(xintercept = filtered_leaders %>% filter(Name == "Benedict XVI" | Name == "Leo XIII") %>% pull(Age.Event))
...
bayesplot::ppc_stat(filtered_leaders %>% filter(Name == "John Paul II") %>% pull(Age.Event),
                    tidybayes::gather_draws(Bayesian_Leaders_V7b %>% as.mcmc(), y_pred[i]) %>% filter(i==2) %>% pull(.value) %>% as.matrix() )
```
