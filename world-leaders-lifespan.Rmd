...
# Covariates (centered)
#
x_1 <- filtered_leaders$time - mean(filtered_leaders$time) # looking at "time" by the decade 
...
modelOutput <- Bayesian_Leaders_V7a$BUGSoutput$summary %>% as.data.frame()
survivalPredicted <- modelOutput[38:210, ] %>% select("mean", "sd", "2.5%", "50%", "97.5%")
...
plot(survivalPredicted$Standardized_Residual, col= survivalPredicted$Censored, pch = shapes)
legend("topright", legend = c("Censored", "Non-Censored"),
       col =  c("black", "red"),
       pch = c(16, 17) )
