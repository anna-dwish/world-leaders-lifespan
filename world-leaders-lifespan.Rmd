...
ggplot(leaders, aes(x=time)) + 
  ...
  labs(title="Distribution of Relative Birth Date")
```
...
ggplot(leaders, aes(x=Age.Event)) + 
  geom_histogram(color="black", fill="white", bins = 10) + 
  labs(title="Distribution of Lifespan")
```

## Distributions of Relative Birth Date Across Leadership Type

```{r}
boxplot(time ~ Type, data = leaders, main="Relative Birth Date vs Type of Leader")
```

## Distributions of Lifespan Across Leadership Type

```{r}
boxplot(Age.Event ~ Type, data = leaders, main="Lifespans vs Type of Leader")
```
...
ggplot(leaders, aes(x=time, y=Age.Event, shape=Type, color=Type)) +
  ...
  ylab("Lifespan") + xlab("Relative Birth Date")
```
...
first_approach <- "
  ...
  for(i in 1:n){
    ...
    beta[i] <- beta_0 + beta_1*x_1[i] + beta_type[x_2[i]]
  }
  ...
  #
  beta_Francis <- beta_0 + beta_1*age_Francis
...
Bayesian_Popes_first_approach <- jags(data = data_Popes_first_approach,  
                                      ...
                                      n.iter = 500, # INCREASE SIMULATIONS
                                      n.chains = 3,
                                      model.file = textConnection(first_approach))

Bayesian_Popes_first_approach$BUGSoutput$sims.matrix %>% 
  as.data.frame() %>% 
  mutate(i = row_number()) %>% 
  tidyr::gather(key, value, "beta_0", "beta_1", "r", "beta_type[2]", "beta_type[3]", "beta_type[4]", "beta_type[5]", -i) %>% 
  ggplot() + 
  geom_line(aes(x = i, y = value, color = key))+ 
  facet_grid(key~., scales = "free_y")


Bayesian_Popes_first_approach$BUGSoutput$sims.matrix[, "survival_Francis"] %>% hist() # this kinda makes sense?? I guess we wnat things to look "approx normal"
```
