...
#present_length <- filtered_leaders$Age.Event
```
...
generate_model_text = function(priors=""){
  ...
  for (i in 1:n){ 
    ...
    y_pred[i] ~  dweib(r, mu[i]) #T(present_length[i], upper_length)
  }
...
leader <- l$Name
censored_value <- as.factor(l$Censored)
actual_survival <- l$Age.Event
time <- l$time
