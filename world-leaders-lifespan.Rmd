...
```{r}
#predictive info for francis, obama, and naruhito, dalai lama
...
leaders_model_V1 <- function() {
  ...
  mu_DL <- exp(beta_DL)
  survival_DL ~ dweib(r, mu_DL) %_% T(present_length_DL, upper_length)
}
```
...
```{r}
if (file.exists("leaders_model_V1.Rdata")) {
  load(file = "leaders_model_V1.Rdata")
} else {
  
  Bayesian_Leaders_V1 <- jags(data = data_leaders,  
                              ...
                              n.iter = 300000, 
                              n.chains = 3,
                              model.file = leaders_model_V1)
  save(Bayesian_Leaders_V1, file = "leaders_model_V1.Rdata")
}
Bayesian_Leaders_V1
```
...
```{r}
post_values_V1 = Bayesian_Leaders_V1$BUGSoutput$sims.matrix %>% 
  as.data.frame() %>% 
  mutate(i = row_number())
...
survival_DL <- Bayesian_Leaders_V1$BUGSoutput$sims.matrix[,"survival_DL"]
survival_Francis <- Bayesian_Leaders_V1$BUGSoutput$sims.matrix[,"survival_Francis"]
...
survival_Obama <- Bayesian_Leaders_V1$BUGSoutput$sims.matrix[,"survival_Obama"]
survival_Naruhito <- Bayesian_Leaders_V1$BUGSoutput$sims.matrix[,"survival_Naruhito"]
...
bugs_for_interpretation <- Bayesian_Leaders_V1$BUGSoutput$summary %>% 
  as.data.frame()
...
## Model 2 - Stronger Priors  

```{r}
leaders_model_V2 <- function() {
  for(i in 1:n){
    censored[i] ~ dinterval(survival[i], censoring_limits[i]) 
    survival[i] ~ dweib(r, mu[i]) # Basic Weibull assumption
    mu[i] <- exp(beta[i])  # Defining beta as log(mu)
...
if (file.exists("leaders_model_V2.Rdata")) {
  load(file = "leaders_model_V2.Rdata")
} else {
  Bayesian_Leaders_V2 <- jags(data = data_leaders,  
                              parameters.to.save = c("beta_0",
                                                     "beta_1",
                                                     "beta_type",
                                                     "beta_int",
                              ...
                              n.iter = 300000, 
                              n.chains = 3,
                              model.file = leaders_model_V2)
  save(Bayesian_Leaders_V2, file = "leaders_model_V2.Rdata")
}

Bayesian_Leaders_V2

Bayesian_Leaders_V2$BUGSoutput$summary %>% 
  as.data.frame() %>% 
  select(-`25%`, -`75%`) %>% 
  knitr::kable()
```
...
```{r}
post_values_V2 = Bayesian_Leaders_V2$BUGSoutput$sims.matrix %>% 
  as.data.frame() %>% 
  mutate(i = row_number())
