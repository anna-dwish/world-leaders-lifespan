...
The code below creates lexis objects in order to display lexis diagrams for the lifespans of leaders across type. More information on this code can be found at this link: http://search.r-project.org/library/Epi/html/plot.Lexis.html
...
plotLifeSpan <- function(leaderType) {
  ...
  plot(lifespans, grid=0:20*5, col='black', xaxs="i", yaxs="i",
       xlim=c(1300,2030), ylim=c(0,100), lwd=3, las=1 , main=plotTitle,
...
tidybayes::gather_draws(Bayesian_Leaders_V6b %>% as.mcmc(), y_pred[i]) %>% 
  ungroup() %>%
  filter(i == 1) %>% 
...
bayesplot::ppc_stat(filtered_leaders %>% filter(Name == "John Paul II") %>% pull(Age.Event),
                    tidybayes::gather_draws(Bayesian_Leaders_V6b %>% as.mcmc(), y_pred[i]) %>% filter(i==2) %>% pull(.value) %>% as.matrix() )
```
...
```{r}
Bayesian_Leaders_output$BUGSoutput$summary["y_pred_censored[102]",]

bayesplot::ppc_stat(filtered_leaders %>% filter(Name == "John Paul II") %>% pull(Age.Event),
                    tidybayes::gather_draws(Bayesian_Leaders_output %>% as.mcmc(), y_pred_censored[i]) %>% filter(i==2) %>% pull(.value) %>% as.matrix() )

p1=tidybayes::gather_draws(Bayesian_Leaders_output %>% as.mcmc(), r, beta_0, beta_1) %>%  
  # tidyr::gather(key, value, "beta_0", "beta_1", "r", -i) %>% 
  ggplot() + 
  geom_line(aes(x = .iteration, y = .value, color = .variable)) + 
  facet_grid(.variable~., scales = "free_y")

p2=tidybayes::gather_draws(Bayesian_Leaders_output %>% as.mcmc(), beta_type[i]) %>% 
  filter(i != 1) %>% 
...
p3=tidybayes::gather_draws(Bayesian_Leaders_output %>% as.mcmc(), beta_int[i]) %>% 
  filter(i != 1) %>% 
  mutate(.variable = ifelse(is.na(i), .variable, paste0(.variable, "[",i, "]"))) %>% 
  #tidyr::gather(key, value, `beta_type[2]`:`beta_type[5]`, -i) %>% 
  ggplot() + 
...
bugs_for_interpretation <- Bayesian_Leaders_output$BUGSoutput$summary %>% 
  as.data.frame()
...
The following code creates a QQ plot and residual plots to evalutate the effectivness of the model in predicting lifespans for the leaders in this dataset. The first residual plot shows the residuals across time to see how well the model is performing for leaders born in different times, while the second residual plot groups the residuals by index, i.e. leadership type, in order to evaluate if the model is accurate across different leadership types.
